...
parser.add_argument('--histos',
        help='Rootfile containing Lumi POG and Run 3 MC pileup distributions',
...
for bin in range (h_nPV_True.GetSize() - 2): #h_nPV_True.GetSize() = 102, removing under/over-flow bin => 100 bins
    if (h_nPV_True.GetBinContent(bin+1) == 0):
        h_weight_nPV_True.SetBinContent(bin+1, 0)
        h_weight_nPV_True.SetBinError  (bin+1, 0)
        l_weights_nPV_True.append(0)
    elif (h_PU_lumiPOG.GetBinContent(bin) == 0):
        h_weight_nPV_True.SetBinContent(bin, 0)
        h_weight_nPV_True.SetBinError  (bin, 0)
        l_weights_nPV_True.append(0)
    else:
        ratio_nPV_True = h_PU_lumiPOG.GetBinContent(bin)/h_nPV_True.GetBinContent(bin+1)
...
h_nPV_True.GetYaxis().SetRangeUser(0, 28000)
h_nPV_True.Draw("histo") 
h_PU_lumiPOG.Draw("sameP hist")
        
# Legend
l2 = TLegend(0.25,0.78,0.9,0.9)
l2.AddEntry(h_nPV_True,"nPV_True in NuGun Run 3 MC", "f")
l2.AddEntry(h_PU_lumiPOG,"PU distribution in Run 3 LumiPOG models","p")
l2.SetFillColor(kWhite)
l2.SetTextFont(43)
l2.SetTextSize(20)
l2.Draw()

c2.SaveAs(outputDir + "/" + "puDistributions_nPVTrue_Run3PU.pdf")
c2.SaveAs(outputDir + "/" + "puDistributions_nPVTrue_Run3PU.png")
