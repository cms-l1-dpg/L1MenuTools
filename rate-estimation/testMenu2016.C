...
int main ( int argc, char *argv[] )
{
  ...
  desc.add_options()
    ...
    ("lowerPUbound",      po::value<int>()->default_value(-1),                    "Consider only events with PU >= lowerPUbound for the rate estimation")
    ("upperPUbound",      po::value<int>()->default_value(-1),                    "Consider only events with PU <= upperPUbound for the rate estimation")
    ("doReweighting2018", po::bool_switch()->default_value(false),                "Reweighting procedure to match the 2018 distribution")
    ("doReweightingRun3", po::bool_switch()->default_value(false),                "Reweighting procedure to use Gaussian Lumi POG models to describe PU at a certain instantaneous luminosity")
    ("customReweighting", po::value<std::string>()->default_value(""),            "Reweighting procedure with custom weights, set the weight json-file")
    ...
    ("doNano",            po::bool_switch()->default_value(false),                "Use nanoAOD format")
    ...
    ("UseFinalDecision", po::bool_switch()->default_value(false),          "Check uGT final decision for prescaled triggers")
